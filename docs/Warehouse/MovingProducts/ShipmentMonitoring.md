...
<h2> Создание сценария работы для АРМа Мониторинг отгрузки </h2>

Сценарий работы используется для хранения настроек в АРМе "Мониторинг отгрузки". Данный справочник расположен в разделе "Склад и доставка" в подсистеме "Складская логистика".

При создании сценария в справочнике заполняются:

Наименование;
Начало и конец периода работы;
Склады отгрузки;
Признак "Упрощенное формирование РнП";
Признак "Использовать задания на доставку";
Признак "Указывать точки доставки";
Описание.

[![22][22]][22]

После выбора сценария нажимаем кнопку "Обновить" и в табличной части ниже появится информация о Планах отгрузки с указанных складов в рамках выбранного периода.

После открытия обработки заполняются или изменяются следующие поля в верхней части рабочей области в разделе **"Настройки"**:
...
Для того, чтобы перевести **"План отгрузки"** в статус **"Набирается"** необходимо распечатать **"План отгрузки"** и передать его на склад. В системе документ переходит в статус **"Набирается"** после нажатия кнопки **"В сборку"**. 
...
[22]: ShipmentMonitoring.assets/22.png
