...
Пример - [работа с заданиями на варку сыров](../../../../Cheese/SemiHardCheese/CookingCheese/TaskFormation/TaskFormation.md).
...
| | Отображать склад получатель | На форме выбора задания отображается поле со складом получателем, указанным в производственном задании | |
| | Включить оповещения | В меню выбора кнопки отображается информация о невыполненном действии | |
...
| | Использовать калькулятор тары | Можно указать вес тары в виде составных частей с заданным в системе фиксированным весом (см.ниже "Пример 2") или вводя вес вручную из обработке ||
...
| Оборудование | Автовзвешивание | Включает\\отключает режим, при котором можно не получать вес с весов, каждый раз нажимая кнопку на форме, а один раз запустить автовзвешивание, после чего поочередно класть продукцию на весы. <br>Каждый "устаканившийся" вес будет равен новому завешиванию в АРМе.| |
...
| | | Контроль отклонения факта от нормы | Позволяет вести контроль отклонения факта от нормы и не разрешает подтверждать учет ингредиентов, если отклонение слишком большое |
| | | Контроль добавления материалов | Включает возможность добавлять и удалять дополнительные ингредиенты только при сканировании бейджа пользователя, у которого есть на это права |
| Выводить расход ингредиентов справочно | потребность в справочном отображении ингредиентов при выпуске для случая, когда нет наборки и материалы списываются по норме. | Указывать основной материал | Допускается указание основного    материала (иначе - только вспомогательные) в АРМе |
| | | Расход ингредиентов | Делает доступной кнопку для указания расхода ингредиентов. Всегда активна при выборе предыдущего параметра. |
| | | Пересчитывать норму ингредиентов по основному материалу | Делает доступным пересчет на форме ингредиентов с помощью калькулятора для справочного отображения. |
| | | Указывать несколько партий материалов | Возможность указывать несколько списываемых партий материалов. Распределяемые партии материалов ограничиваются указанным списком. |
| | | Вариант распределения для партий материалов - "В порядке выбора" | Возможность списывать указанную (отсканированную) партию материала при выпуске. |
| Отсутствие галочек по наборке и учету ингредиентов | Позволяет указывать выпуск продукции по заданию с или без списания материалов по норме | Основные материалы. Количество | Правило для списания количества основного материала <br>По выпуску, материал из спецификации - количество по выпуску, серия равна серии выпуска <br>Все остатки - все остатки основного материала по спецификации на складе-отправителе |
...
- **Использовать на ТСД** – интерфейс работы с КУТом меняется и подстраивается под работу на ТСД

![](WorkWithTasks.assets/UseTSD.png)

- **Свертывание строк производственного задания** - позволяет визуально свернуть несколько строк ПЗ с одинаковой номенклатурой в одну. Таким образом оператор сможет сначала ознакомиться с общим выпуском, а затем выбрать необходимую номенклатуру и приступить к выполнению по каждой строке задания.

![](WorkWithTasks.assets/1.png)
