...
Выбрать необходимую номенклатуру можно путем сканирования этикетки партии. Если строк окажется неколько, то будет применен отбор для ручного выбора. Также стоит отметить, что в случае выполнения плана по перемещению, строки будут пропадать.
...
- **Контролировать перемещение** – В документе Распоряжение на перемещение заполняется только план перемещения. Оператору будет выводиться информация об отправленном количестве. Для подтверждения факта необходимо подтверждение с принимающей стороны.
...
- **Проверять соответствие номенклатуре упаковочного листа** – при несоответствии номенклатуры тому, что указано в упаковочном листе будет показано уведомление об ошибке. Также в случае необходимости выбора партии будет доступна только одна наиболее подходящая партия. 
...
- **Использовать на ТСД** – адаптация интерфейса обработки под работу на ТСД.
