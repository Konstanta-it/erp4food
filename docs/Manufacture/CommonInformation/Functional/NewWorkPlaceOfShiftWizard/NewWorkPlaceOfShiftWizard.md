...
- Признак *"Отслеживать перемещения"*
...
<h2> Вкладка "Перемещения" </h2>

На вкладке Перемещения отображаются запланированное, отправленное и принятое количество по заказам на перемещение со склада РЦ.  

Таблицу можно сгруппировать по получателям (складам получателям) или по номенклатуре.  

Чтобы открыть интересующий Заказ на перемещение, необходимо выбрать сроку и нажать "Перемещение" - "Открыть". Для того, чтобы создать новый Заказ на перемещение нужно нажать "Перемещение" - "Создать".

Запланировано заполняется товарами из документа [Заказ на перемещение](../../../../Warehouse/MovingProducts/TransferOrder.md) (где склад отправитель - склад РЦ)  

Отправлено заполняется товарами из документа [Распоряжение на отгрузку](../../../../Warehouse/MovingProducts/ShipmentMonitoring.md) или вкладкой План документа Распоряжение на перемещение с типом Перемещение (у которых основание - [Заказ на перемещение](../../../../Warehouse/MovingProducts/TransferOrder.md) из Запланировано)  

Принято заполняется товарами из документа [Распоряжение на приемку](../../../../Warehouse/MovingProducts/Acceptance.md) (с основанием [Заказ на перемещение](../../../../Warehouse/MovingProducts/TransferOrder.md) или [Перемещение товаров](../../../../Warehouse/MovingProducts/TransferProducts.md) по заказу из Запланировано) или вкладкой Факт документа Распоряжение на перемещение с типом Перемещение (у которого основание - [Заказ на перемещение](../../../../Warehouse/MovingProducts/TransferOrder.md) из Запланировано)  

_Отклонение = (Запланировано - Принято) * 100 / Запланировано_

![](NewWorkPlaceOfShiftWizard.assets/Movements.png) 
...
3. Созданная объединением партия получает параметр "Объединение". Данный параметр символизирует о том, что данная партия была получена путем объединения, а также фильтром при формировании отчета "Баланс жира и белка", где данные партии не должны учитываться.
![](NewWorkPlaceOfShiftWizard.assets/20.png)
...
В случаях, когда необходимо списывать отрицательные отстатки, нужно указать функциональную опцию "Разрешить списание отрицательного остатка" в настройках рабочего центра на вкладке "Производство". Если данная ФО активна на выбранном РЦ, то система позволит выполнить списание отрицательных остатков как в рамках смены, так и в рамках партии.
...
- **Пропорционально списанному по смене** - требует наличие выпуска в рамках партии, где материал, по которому есть остаток, уже был потрачен.
    -   Выбрать материал (и серию) - нажать **"Списать"**. Указать вариант списания "Пропорционально списанному по партии".

- **Пропорционально списанному по партии** - требует наличие выпуска, где материал, по которому есть остаток, уже был потрачен.
...
<h3> Отчет "Анализ остатков на складах с регистраторами остатков" </h3>

Данный отчет содержит в себе информацию об остатках выбранной партии на складах. Отчет открывается по кнопке, которая находится на вкладке "Остатки" при выборе интересующей партии.
